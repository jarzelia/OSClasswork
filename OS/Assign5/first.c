...
int allocateMemory(struct request* request)
{
    //create free list
    ...
    for(free = list_head.next; free; free = free -> next)
    {
        if(request -> size <= free -> block_size)
        {
            ...
            request -> base_adr = free -> block_adr;
            ...
            if((free -> block_size = free -> block_size - request -> size) == 0)
            {
            ...
            free -> block_adr = free -> block_adr + request -> size;
            return 0;
...
int first(int mem, char* fileIn)
{
    ...
    while(fscanf(file, "%d %s %d", &reqSeq, &reqType, &reqSize) != EOF)
    {
        ...
        if(strcmp(reqType, "alloc") == 0 )
        {
            ...
            allocateMemory(&req_array[reqSeq]);
    ...
    //print results
    printf("POLICY: First\tMEMORY SIZE: %d\n\n", mem);
